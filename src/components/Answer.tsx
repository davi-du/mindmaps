...
// Importazione delle icone per l'interfaccia
...
const AnswerListView = ({
  ...
  const [diagramDisplay, setDiagramDisplay] = useState<DiagramDisplayFormat>('split')

  const handleSwitchDiagramDisplay = useCallback((newDisplayFormat: DiagramDisplayFormat) => {
    ...
    if (answerObjectElement) {
      answerObjectElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
    }
  }, [handleSetSyncedAnswerObjectIdsHidden, handleSetSyncedAnswerObjectIdsHighlighted, id])
  ...
  const handleHighlightAnswerObject = useCallback(
    (answerObjectId: string, addOrRemove: 'add' | 'remove', temp: boolean) => {
      ...
      if (addOrRemove === 'remove') {
        ...
        else handleSetSyncedAnswerObjectIdsHighlighted(currentIds.filter(id => id !== answerObjectId), false)
      } else {
        if (temp) handleSetSyncedAnswerObjectIdsHighlighted([answerObjectId], true)
        else handleSetSyncedAnswerObjectIdsHighlighted([...currentIds, answerObjectId], temp)
      }
    },
    [handleSetSyncedAnswerObjectIdsHighlighted, synced.answerObjectIdsHighlighted, synced.answerObjectIdsHighlightedTemp],
  )

  const handleHideAnswerObject = useCallback((answerObjectId: string) => {
    const currentIds = synced.answerObjectIdsHidden

    if (currentIds.includes(answerObjectId)) {
      handleSetSyncedAnswerObjectIdsHidden(currentIds.filter(id => id !== answerObjectId))
    } else {
  ...
  return (
    ...
    >
      <div className="answer-item-display" data-id={id}>
        <div className="block-display-switches">
          <div className="list-display-switch">
            <button
              className={`bar-button${diagramDisplay === 'split' ? ' selected' : ''}`}
            ...
            <button
              className={`bar-button${diagramDisplay === 'merged' ? ' selected' : ''}`}
          ...
          >
            {saliencyFilter === 'high' ? (
              <SignalWifi1BarRoundedIcon style={{ transform: 'rotate(180deg)' }} />
            ) : (
              <SignalWifi4BarRoundedIcon style={{ transform: 'rotate(180deg)' }} />
            )}
            <span>saliency</span>
          </button>
        ...
        <div className={`answer-block-list${diagramDisplay === 'merged' ? '-merged-diagram' : ''}`} data-id={id}>
          {diagramDisplay === 'merged' ? (
            ...
            answerObjects.map((answerObject, index) => (
              <ReactFlowProvider key={`answer-block-flow-provider-${id}-${answerObject.id}`}>
                <AnswerBlockItem
                  key={`answer-block-item-${id}-${answerObject.id}`}
                  index={index}
                  questionAndAnswer={questionAndAnswer}
