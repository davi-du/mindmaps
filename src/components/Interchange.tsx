...
export const Interchange = ({
  ...
  const setQuestionsAndAnswers = contextValue.setQuestionsAndAnswers;
    ...
    async (answerObject: AnswerObject): Promise<string> => {
      ...
      await Promise.all(
        jobs.map(async ({ sentence, n, e }) => {
          const correctionResponse = await getAICompletion(
  ...
  const handleStreamRawAnswer = useCallback(
    (data: AIChatCompletionResponseStream, newParagraph: boolean) => {
  ...
  */
  ...
  const handleAnswerObjectsAddOneMore = useCallback(async () => {
    ...
    const previousAnswerPrompt: Prompt = {
      role: 'assistant',
      content: answer,
    }

    // costruisci conversazione da passare a _1MoreParagraph
    const prevConversation: Prompt[] = [
      contextPrompt,
      questionPrompt,
      previousAnswerPrompt,
    ]
